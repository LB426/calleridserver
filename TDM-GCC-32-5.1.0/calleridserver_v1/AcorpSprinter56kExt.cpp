...
std::string AcorpSprinter56kExt::MdmLineToHumanString(std::string mline)
{
    ...
    for(;;)
    {
        ...
        while (read_char(c))
        {
            ...
            if(boost::regex_match(rsp,xResults,myRegEx))
            {
                ...
                logMsg(ss.str());
                return; // прерываем цикл ожидание номера для отправки номера, цикл возобновиться извне, поэтому rsp строка накопитель не очищается
            }
        }
        if(timeoutexpired){
            std::stringstream ss;
