    ...
    for (int i = 1;; i++) {
      if (rdrs.size() == 0) {
        ...
        for (int j = 0; j < rdrs.size(); j++) {
          //if (rdrs[j]->thread_completed == false) {
          ...
          /*} else {
            ...
            ss << "определитель №: " << threadNumber << ", "                
               << "готовность 1, завершённый поток: " << rdrs[j]->id << ", "
        ...
        if (noready.size() != 0) {
          for (int j=0; j < noready.size(); j++ ) {
            std::stringstream ss;
            ss << "noready[" << j << "]=" << noready[j] << ", " << "rdrs.size=" << rdrs.size();
            logMsg(ss.str());
            ...
            for( it = rdrs.begin(); it != rdrs.end(); it++ ) {
                if( (*it)->thread_completed == true ) {
                    myThreadInfo *mtPtr = (*it);
                    std::stringstream ss;
                    ss << "после готовности 2, стираю myThreadInfo для потока: " << mtPtr->id << " и удаляю из rdrs";
                    logMsg(ss.str());
                    delete(mtPtr);
                    rdrs.erase(it);
                    break;
